...
//  MacroCaster
//
//  Created by sunshine.lee on 2018/4/4.
...
//        bytes.deallocate(capacity: length)
    ...
    func hmac(algorithm: CryptoAlgorithm, key: String,outRawData:Bool) -> Any {
        let str = self.cString(using: String.Encoding.utf8)
...
//            result.deallocate(capacity: digestLen)
    ...
    private func stringBinaryFromResult(result: UnsafeMutablePointer<CUnsignedChar>, length: Int) -> String {
        let hash = NSMutableString()
        for i in 0..<length {
            hash.appendFormat("%02x", result[i])
        }
